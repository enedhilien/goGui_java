namespace gogui {
  ...
  class ProcessedLine : public Line, public AbstractContainer {
   public:
  	ProcessedLine(const Line& line) : Line(line)
  	{
  	  setStatus(Status::Processed);
  	}
  	ProcessedLine(const Point& p1, const Point& p2) : ProcessedLine(Line(p1, p2))
  	{}
